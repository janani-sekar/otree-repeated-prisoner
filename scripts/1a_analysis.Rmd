...
n_participants <- n_distinct(paired_df$`player.prolific_id`)
n_game_configs <- paired_df %>%
  distinct(
    group.game_payoff_cooperate_cooperate,
    group.game_payoff_betrayed,
    group.game_payoff_betray,
    group.game_payoff_both_defect,
    group.delta_value
  ) %>%
  nrow()
seq_lengths <- map_int(paired_df$decision_tuple, length)
avg_seq_length <- mean(seq_lengths)
median_seq_length <- median(seq_lengths)
p25_seq_length <- quantile(seq_lengths, 0.25)
p75_seq_length <- quantile(seq_lengths, 0.75)
p95_seq_length <- quantile(seq_lengths, 0.95)

tibble(
  Statistic = c(
    "Total sequences",
    "Unique participants",
    "Unique (payoff matrix, δ) combinations",
  ...
  Value = c(
    ...
    sprintf("%.0f rounds", median_seq_length),
    sprintf("%.0f rounds", p25_seq_length),
    sprintf("%.0f rounds", p75_seq_length),
    sprintf("%.0f rounds", p95_seq_length)
  )
) %>%
  knitr::kable()
```
...
```{r surplus-histogram, fig.width=8, fig.height=4}
...
```{r surplus-cooperation, fig.width=8, fig.height=4}
surplus_df <- paired_df %>%
  select(surplus_left, p1_coop_rate, p1_r1_coop, delta_rd, delta_spe, group.delta_value)

# Surplus vs P1 coop rate
ggplot(surplus_df, aes(x = p1_coop_rate, y = surplus_left)) +
  geom_point(alpha = 0.3) +
  labs(
    x = "P1 Cooperation Rate",
    y = "Surplus Left",
    title = "Surplus Left vs P1 Cooperation Rate"
  ) +
  theme_bw()

ggsave("../figures/surplus_vs_p1_coop_rate.png", width = 6, height = 5)
```
...
```{r transition-matrix}
## Transition matrix over joint actions
...
### P1 Round 1 Cooperation \~ Delta_rd
...
next_p1_initial_lpm <- lm(
  next_p1 ~ Delta_rd + g + l + `group.delta_value`,
...
next_p1_noninitial_lpm <- lm(
  next_p1 ~ previous_pair + Delta_rd + g + l + `group.delta_value` + round_next,
